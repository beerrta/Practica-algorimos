function [pob]=poblacions(portes,poblacio)
n=1;
pob=[];
%portes=8
...
for i=1:length(A(:,1)) %recorro files
 ...
 %el 5 es perque en la porta 4 hi ha els cables llavors a partir de la 5ena posem els cables randoms ja uqe en la 4 ho hem fet anteriormet
            if i>j && sum(A(:,i-1))
                A(j,i)=round(rand(1));
            end
    end  
